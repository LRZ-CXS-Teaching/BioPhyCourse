...
|-------------|-------------|-------------|------------|-----------------------|-------|
| gpu-node    | 640         |  5120       |  16GB      |  Tesla V100-PCIE-16GB |  2    |
...
mpirun -np 80 cp2k.psmp -i H2O-32.inp > cp2k.out 2>&1
```
...
#SBATCH --output=cp2k_%j.log      # Standard output (%j = JobID)
#SBATCH --error=cp2k_%j.err       # Capture errors separately
...
For visualization use ood that provides desktop environment: https://vm-138-246-237-46.cloud.mwn.de:8443/ 

Click on "Remote Desktop Container". Select sufficient number of cores and RAM. Start the desktop. 

Once desktop is ready, click the terminal and follow respective instructions below. 
